#ifndef OSMIUM_UTIL_CHECK_ORDER_HPP
#define OSMIUM_UTIL_CHECK_ORDER_HPP
...
namespace osmium {
    ...
    /**
     * Helper that can be used to check that an OSM file is ordered correctly.
     * Ordered in this case refers to the usual order in OSM files: First
     * nodes in the order of their IDs, then ways in the order of their IDs,
     * then relations in the order or their IDs. IDs have to be unique for
     * each type.
     *
     * To use this add a CheckOrder member variable to your handler and call
     * the node(), way(), and relation() methods from your node(), way(), and
     * relations() handlers, respectively. An out_of_order_error exception
     * will be thrown when the input is not in order.
     */
    class CheckOrder {
    ...
    public:
        ...
        osmium::object_id_type max_node_id() const {
            return m_max_node_id;
        }

        osmium::object_id_type max_way_id() const {
            return m_max_way_id;
        }

        osmium::object_id_type max_relation_id() const {
...
#endif // OSMIUM_UTIL_CHECK_ORDER_HPP
